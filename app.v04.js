...
const defaultData = {
  ...
  settings: { frameMinutes: 5, parentPin: '1234', familyName: 'Familie Becker' }
};
...
function saveData() { localStorage.setItem(STORE_KEY, JSON.stringify(data)); renderAvatars(); }
function escapeHtml(text) { return String(text).replace(/[&<>"']/g, m => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m])); }
function byPerson(name) { return data.family.find(f => f.name === name) || { color: '#e5e7eb', emoji: '👤', initial: '?' }; }
function options(selected='') { return data.family.map(p => `<option ${p.name===selected?'selected':''}>${escapeHtml(p.name)}</option>`).join(''); }
function setActiveNav(label) { document.querySelectorAll('.nav').forEach(b => b.classList.remove('active')); const map = { calendar:0, school:1, chores:2, points:3, meals:4, shopping:5, accounts:6, parents:7 }; const idx = map[label]; if (idx !== undefined) document.querySelectorAll('.nav')[idx].classList.add('active'); }
function updateClock() { document.getElementById('clock').textContent = new Date().toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' }); const familyTitle = document.getElementById('familyTitle'); if (familyTitle) familyTitle.textContent = data.settings.familyName || 'Familie'; }
setInterval(updateClock, 1000); updateClock();

function renderAvatars() { document.getElementById('familyAvatars').innerHTML = data.family.map(p => `<div class="avatar" style="--color:${p.color}" title="${escapeHtml(p.name)}">${p.photo ? `<img src="${p.photo}" alt="${escapeHtml(p.name)}" style="width:100%;height:100%;object-fit:cover;border-radius:50%">` : p.emoji}</div>`).join('') + `<button class="gear" onclick="openModule('parents')">⚙️</button>`; }
...
function renderDashboard() {
  ...
  for (let r=0; r<12; r++) {
    ...
    for (let c=0; c<5; c++) html += `<div class="cell" data-day="${c+1}" data-row="${r+1}" style="grid-column:${c+2};grid-row:${r+2}"></div>`;
  }
  ...
  data.events.forEach(e => {
    ...
    html += `<div class="event" draggable="true" data-event-id="${e.id}" style="left:${left};top:${top};width:${width};height:${height};background:${e.color};"><strong>${escapeHtml(e.title)}</strong><small>${escapeHtml(e.time)}</small><span class="tag">${escapeHtml(p.initial || '?')}</span></div>`;
  });
  html += `</section>`;
...
function enableCalendarInteractions() {
  ...
  document.querySelectorAll('.event').forEach(eventEl => {
    eventEl.addEventListener('click', event => {
      ...
      clickTimer = setTimeout(() => editCalendarEvent(eventEl.dataset.eventId), 120);
    });
    eventEl.addEventListener('dragstart', event => { draggedEventId = eventEl.dataset.eventId; eventEl.classList.add('dragging'); event.dataTransfer.setData('text/plain', draggedEventId); });
    eventEl.addEventListener('dragend', () => { eventEl.classList.remove('dragging'); document.querySelectorAll('.cell').forEach(cell => cell.classList.remove('drop-target')); });
  });
  document.querySelectorAll('.cell').forEach(cell => {
    cell.addEventListener('dragover', event => { event.preventDefault(); cell.classList.add('drop-target'); });
    cell.addEventListener('dragleave', () => cell.classList.remove('drop-target'));
    cell.addEventListener('drop', event => { event.preventDefault(); cell.classList.remove('drop-target'); const id = event.dataTransfer.getData('text/plain') || draggedEventId; moveCalendarEvent(id, Number(cell.dataset.day), Number(cell.dataset.row)); });
  });
}
function moveCalendarEvent(id, day, row) { const e = data.events.find(event => event.id === id); if (!e) return; e.day = day; e.row = row; saveData(); renderDashboard(); }

function editCalendarEvent(id) {
  const e = data.events.find(event => event.id === id); if (!e) return;
  setDialog('Termin bearbeiten', `<div class="panel"><div class="form-row"><input id="editEventTitle" value="${escapeHtml(e.title)}" /><input id="editEventTime" value="${escapeHtml(e.time)}" /><button class="action-btn" onclick="saveCalendarEvent('${e.id}')">Speichern</button></div><div class="form-row"><select id="editEventPerson">${options(e.person)}</select><select id="editEventDay"><option value="1" ${e.day===1?'selected':''}>Montag</option><option value="2" ${e.day===2?'selected':''}>Dienstag</option><option value="3" ${e.day===3?'selected':''}>Mittwoch</option><option value="4" ${e.day===4?'selected':''}>Donnerstag</option><option value="5" ${e.day===5?'selected':''}>Freitag</option></select><select id="editEventColor"><option value="#f8dfb8" ${e.color==='#f8dfb8'?'selected':''}>Creme</option><option value="#f6b5b2" ${e.color==='#f6b5b2'?'selected':''}>Rosa</option><option value="#a8dedc" ${e.color==='#a8dedc'?'selected':''}>Türkis</option><option value="#9691e8" ${e.color==='#9691e8'?'selected':''}>Violett</option><option value="#bfe8c7" ${e.color==='#bfe8c7'?'selected':''}>Grün</option></select></div><div class="form-row"><input id="editEventRow" type="number" min="1" max="12" value="${e.row}" /><input id="editEventSpan" type="number" min="0.5" max="8" step="0.5" value="${e.span}" /><button class="danger-btn" onclick="deleteCalendarEvent('${e.id}')">Löschen</button></div></div>`);
}
function saveCalendarEvent(id) { const e = data.events.find(event => event.id === id); if (!e) return; e.title = document.getElementById('editEventTitle').value.trim() || e.title; e.time = document.getElementById('editEventTime').value.trim(); e.person = document.getElementById('editEventPerson').value; e.day = Number(document.getElementById('editEventDay').value); e.color = document.getElementById('editEventColor').value; e.row = Number(document.getElementById('editEventRow').value); e.span = Number(document.getElementById('editEventSpan').value); saveData(); closeModule(); renderDashboard(); }
function deleteCalendarEvent(id) { if (!confirm('Termin wirklich löschen?')) return; data.events = data.events.filter(event => event.id !== id); saveData(); closeModule(); renderDashboard(); }

function openModule(key) { const modules = { calendar: () => { setActiveNav('calendar'); renderCalendar(); }, school: () => { setActiveNav('school'); renderSchool(); }, chores: () => { setActiveNav('chores'); renderChores(); }, points: () => { setActiveNav('points'); renderPoints(); }, meals: () => { setActiveNav('meals'); renderMeals(); }, shopping: () => { setActiveNav('shopping'); renderShopping(); }, accounts: () => { setActiveNav('accounts'); renderAccounts(); }, parents: () => { setActiveNav('parents'); renderParentLogin(); }, quickAdd: renderQuickAdd }; modules[key](); }
function closeModule() { document.getElementById('moduleDialog').close(); }
function setDialog(title, html) { document.getElementById('dialogTitle').textContent = title; document.getElementById('dialogContent').innerHTML = html; document.getElementById('moduleDialog').showModal(); }
function renderCalendar() { setDialog('Kalender-Termin hinzufügen', `<div class="panel"><div class="form-row"><input id="eventTitle" placeholder="Termin" /><input id="eventTime" placeholder="Zeit" /><button class="action-btn" onclick="addEvent()">Speichern</button></div><div class="form-row"><select id="eventPerson">${options()}</select><select id="eventDay"><option value="1">Montag</option><option value="2">Dienstag</option><option value="3">Mittwoch</option><option value="4">Donnerstag</option><option value="5">Freitag</option></select><select id="eventColor"><option value="#f8dfb8">Creme</option><option value="#f6b5b2">Rosa</option><option value="#a8dedc">Türkis</option><option value="#9691e8">Violett</option></select></div></div>`); }
function addEvent() { const title = document.getElementById('eventTitle').value.trim(); if (!title) return; data.events.push({ id: crypto.randomUUID(), title, time: document.getElementById('eventTime').value, day: Number(document.getElementById('eventDay').value), row: 5, span: 1.5, person: document.getElementById('eventPerson').value, color: document.getElementById('eventColor').value, icon: '📌' }); saveData(); renderDashboard(); closeModule(); }
function renderSchool() { document.getElementById('content').innerHTML = `<div class="panel-grid"><div class="panel"><h2>🎓 Schule</h2><div class="item">📚 Hausaufgaben lesen</div><div class="item">✍️ Mathe üben</div><div class="item">🎒 Ranzen packen</div></div><div class="panel"><h2>Heute wichtig</h2><p class="pill">Sportzeug</p><p class="pill">Unterschrift</p><p class="pill">Brotdose</p></div></div>`; }
function renderChores() { document.getElementById('content').innerHTML = `<div class="panel-grid"><div class="panel"><h2>✅ Aufgaben</h2>${data.chores.map(c => { const p = byPerson(c.person); return `<div class="item ${c.done?'done':''}"><div class="item-left"><span class="dot" style="background:${p.color}"></span><button class="big-check" onclick="toggleChore('${c.id}')">${c.done?'✓':'○'}</button>${c.icon} ${escapeHtml(c.title)} <span class="pill">${escapeHtml(c.person)}</span><span class="pill">⭐ ${c.points}</span></div></div>`; }).join('')}</div><div class="panel"><h2>Neue Aufgabe</h2><div class="form-row"><input id="choreTitle" placeholder="Aufgabe" /><select id="chorePerson">${options()}</select><button class="action-btn" onclick="addChore()">Speichern</button></div></div></div>`; }
function addChore() { const title = document.getElementById('choreTitle').value.trim(); if (!title) return; data.chores.push({ id: crypto.randomUUID(), title, person: document.getElementById('chorePerson').value, points: 1, done: false, icon: '✅' }); saveData(); renderChores(); }
function toggleChore(id) { const c = data.chores.find(x => x.id === id); if (!c) return; c.done = !c.done; if (c.done) data.points[c.person] = (data.points[c.person] || 0) + c.points; saveData(); renderChores(); }
function renderPoints() { document.getElementById('content').innerHTML = `<div class="panel-grid">${data.family.map(p => `<div class="panel"><h2><span class="dot" style="background:${p.color};display:inline-block;vertical-align:middle"></span> ${p.emoji} ${escapeHtml(p.name)}</h2><p style="font-size:4rem;font-weight:900;margin:0">⭐ ${data.points[p.name] || 0}</p><button class="action-btn" onclick="addPoint('${escapeHtml(p.name)}')">+1 Stern</button></div>`).join('')}</div>`; }
function addPoint(name) { data.points[name] = (data.points[name] || 0) + 1; saveData(); renderPoints(); }
function renderMeals() { document.getElementById('content').innerHTML = `<div class="panel-grid"><div class="panel"><h2>🍽️ Essensplan</h2>${data.meals.map(m => `<div class="item"><div>${m.icon} <strong>${escapeHtml(m.day)}</strong>: ${escapeHtml(m.meal)}</div></div>`).join('')}</div></div>`; }
function renderShopping() { document.getElementById('content').innerHTML = `<div class="panel-grid"><div class="panel"><h2>🛒 Einkaufsliste</h2>${data.shopping.map(s => `<div class="item ${s.done?'done':''}"><div class="item-left"><button class="big-check" onclick="toggleShop('${s.id}')">${s.done?'✓':'○'}</button>${escapeHtml(s.title)}</div></div>`).join('')}</div><div class="panel"><h2>Artikel hinzufügen</h2><div class="form-row single"><input id="shopTitle" placeholder="z. B. Bananen" /><button class="action-btn" onclick="addShop()">Speichern</button></div></div></div>`; }
...
renderAvatars(); renderDashboard(); resetIdleTimer();
